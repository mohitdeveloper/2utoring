        ...
        ['$scope', '$timeout', '$rootScope', '$filter', 'ModalService', 'DeleteService', 'SessionGroupsService', 'SessionWhiteBoardsService', 'ClassSessionsService',
            function ($scope, $timeout, $rootScope, $filter, ModalService, DeleteService, SessionGroupsService, SessionWhiteBoardsService, ClassSessionsService) {
                ...
                $scope.mouseDown = function (e, index) {
                    ...
                    var posMouseDown = $scope.getElementRelativeMousePosition(e, $scope.whiteboardTabs[index].canvas, index);
                    ...
                    $scope.whiteboardTabs[index].last_mousex = $scope.whiteboardTabs[index].mousex =  posMouseDown.x;
                    $scope.whiteboardTabs[index].last_mousey = $scope.whiteboardTabs[index].mousey =  posMouseDown.y;
                ...
                $scope.mouseMove = function (e, index) {
                    ...
                    var posMouseMove = $scope.getElementRelativeMousePosition(e, $scope.whiteboardTabs[index].canvas, index);
                    ...
                    $scope.whiteboardTabs[index].mousex = posMouseMove.x;
                    $scope.whiteboardTabs[index].mousey = posMouseMove.y;
                ...
                $scope.drawTempHoldMove_Step2 = function (index, start_x, start_y, x, y, width) {
                    ...
                    $scope.whiteboardTabs[index].tempCtx.font = "bold 20px";

                    $scope.whiteboardTabs[index].tempCtx.fillText('|', x, y + 20);

                    $scope.getActuallPositoion(index, start_x, start_y, x, y);
                    //console.log(x, 'remove item');
                ...
                $scope.getElementRelativeMousePosition = function (e, elem, index) {
                    //for firefox
                    if (navigator.userAgent.indexOf('Firefox') != -1 && parseFloat(navigator.userAgent.substring(navigator.userAgent.indexOf('Firefox') + 8)) >= 3.6) {
                        return {
                            x: parseInt(e.offsetX / $scope.zoomSet[$scope.whiteboardTabs[index].zoomIndex]),
                            y: parseInt(e.offsetY / $scope.zoomSet[$scope.whiteboardTabs[index].zoomIndex]),
                        };

                    } else {
                        //for other like chrome, edge, safari where ever that zoom works
                        const rect = elem.getBoundingClientRect();
                        const css = getComputedStyle(document.body);
                        return {
                            x: parseInt(e.clientX / css.zoom - rect.left),
                            y: parseInt(e.clientY / css.zoom - rect.top),
                        };
                    }
                }

                $scope.getActuallPositoion = function (index, start_x, start_y, x, y) {
                    
                    document.onkeydown = function (e) {
                        switch (e.keyCode) {
                            case 37:
                                $scope.adustCursorForText('leftArrowKey', index, start_x, start_y, x, y);
                                break;
                            case 38:
                                //$scope.whiteboardTabs[index].tempCtx.fillText('|', x, y + 20);
                                //str = 'Up Key pressed!';
                            ...
                            case 40:
                                ...
                                break;
                            case 39:
                                $scope.adustCursorForText('rightArrowKey', index, start_x, start_y, x, y);
                                str = 'Right Key pressed!';
                                break;
                            case 8:
                                str = 'Back space Key pressed!';
                                $scope.whiteboardTabs[index].tempCtx.fillText('|', x, y + 20);
                                $scope.removeLastCharater+2;
                                break;
                            case 46:
                                //return false;
                                str = 'Delete Key pressed!';
                                $scope.whiteboardTabs[index].tempCtx.fillText('|', x, y + 20);
                                $scope.removeLastCharater--;
                                break;
                        }
                    }

                    //document.onkeyup = function (e) {
                    //   // $scope.adustCursorForText('default', index, start_x, start_y, x, y);
                    //}

                }


                $scope.adustCursorForText = function (buttonType, index, start_x, start_y, x, y) {
                    if (buttonType == 'leftArrowKey') {
                        if ($scope.whiteboardTabs[index].text.length == $scope.removeLastCharater) {
                            $scope.removeLastCharater = $scope.whiteboardTabs[index].text.length;
                        } else {
                            $scope.removeLastCharater++;
                        }
                    } else if (buttonType == 'rightArrowKey'){
                        if ($scope.removeLastCharater == 0) {
                            return false;
                        } else {
                            $scope.removeLastCharater--;
                        }
                    ...
                    console.log($scope.removeLastCharater, 'ff');
                    let originalWidth = Math.ceil($scope.whiteboardTabs[index].tempCtx.measureText($scope.whiteboardTabs[index].text).width);
                    let newWidth = Math.ceil($scope.whiteboardTabs[index].tempCtx.measureText($scope.whiteboardTabs[index].text.slice(0, -$scope.removeLastCharater)).width);
                    console.log(originalWidth, newWidth);
                    let adjustedXposition = start_x + newWidth;
                    //if (newWidth >= originalWidth) {
                    //    adjustedXposition = start_x;
                    //    //$scope.removeLastCharater = 0;
                    //}
                    //if (newWidth <= 0) {
                    //    adjustedXposition = start_x;
                    //    //$scope.removeLastCharater = 0;
                    //}
                    $scope.whiteboardTabs[index].tempCtx.fillText('|', adjustedXposition, y + 20);
                    str = buttonType;
                    console.log(str)
                }
