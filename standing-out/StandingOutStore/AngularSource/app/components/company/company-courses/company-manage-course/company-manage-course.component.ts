...
export class CompanyManageCourseComponent implements OnInit {
    ...
    errorObj = [{ 'key': 4, 'value': 'Please select level!' }, { 'key': 6, 'value': 'Please click next to save calendar changes!' }, { 'key': 7, 'value': 'Please enter course details!' }, { 'key': 9, 'value': 'Please enter lesson details!' }];
    ...
    ngOnInit(): void {
        ...
        this.companyService.checkCompanyUsersHasDBS().subscribe(success => {
            this.isDBSApprove = success;
        }, error => {
        });
    ...
    getAllSubject() {
        ...
        //    }, error => {
        //    });
        this.companyService.getCompanyTutorsSubject().subscribe(success => {
            this.subjectData = success;
            $('.loading').hide();
        }, error => {
        });
    ...
    getStudyLevels(id) {
        ...
        //    }, error => {
        //    });
        this.companyService.getCompanyTutorsLevelBySubject(id).subscribe(success => {
            this.StudyLevels = success;
            $('.loading').hide();
        }, error => {
        });
    ...
    getAllTutors() {
        ...
        this.companyService.getAllCompanyTutors(subjectData)
            .subscribe(success => {
                if (success.length > 0) {
                    $('.loading').hide();
                    this.selectedTutorId = success[0].tutorId;
                    this.tutorsData = success;
                    this.getSelectedTutorDetails(this.selectedTutorId);
                } else {
    ...
    searchTutorByAvailability(inputSearchTime, totalRequiredSlot) {
            ...
            this.companyService.getTutorByAvailability(this.tutorSearchQuery)
                .subscribe(success => {
                    if (success != null) {
                        if (success.length > 0) {
                            this.tutorsData = success;
                            this.getSelectedTutorDetails(this.tutorsData[0].tutorId);
                            debugger;
                            $('.loading').hide();
                        } else {
                            this.toastr.warning('No tutors found for this selected time slots!');
                            this.getAllTutors();
                            $('.loading').hide();
                        }
                    }
    ...
    onChangeWeek($event) {
        let dayOfWeek = $event.target.options[$event.target.options.selectedIndex].value;
    ...
    getSelectedTutorDetails(tutorId) {
        $('.loading').show();
        this.companyService.getTutorsDetails(tutorId)
            .subscribe(success => {
                this.selectedTutorData = success;
                this.getTutorAvailability(tutorId);
                this.CompanyCourseForm.controls["tutorId"].setValue(tutorId);
    ...
    getSelectedTutorIdFromOnTutorSelection($event) {
        let id = $event.target.options[$event.target.options.selectedIndex].value;
        if (!localStorage.getItem('courseId')) {
            this.selectedEvent = [];
            this.classSessions.clear();
    ...
    exitCourse() {
        ...
        dialogRef.afterClosed().subscribe((showSnackBar: boolean) => {
            if (showSnackBar) {
                this.submitCompanyCourseForm(13)
            }
            else {
                window.location.href = "/admin/courses"
            }
