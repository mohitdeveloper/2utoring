...
namespace StandingOut.XUnitTests.Base
{
    public class BaseClient
    {
        ...
        public BaseClient(ITestOutputHelper output)
        {
            _output = output;
        }
        ...
        public void Initialise(int port)
        {
            ...
            {
                case "IE":
                    InternetExplorerDriverService ieService = InternetExplorerDriverService.CreateDefaultService(System.IO.Directory.GetCurrentDirectory());
                    ieService.Port = port;
                    var IEOptions = new InternetExplorerOptions();
                    _driver = new InternetExplorerDriver(ieService, IEOptions);
                    break;
                case "Edge":
                    ...
                    break;
                case "Chrome":
                    ChromeDriverService service = ChromeDriverService.CreateDefaultService(System.IO.Directory.GetCurrentDirectory());
                    service.Port = port;
                ...
                case "Firefox":
                default:
                    FirefoxDriverService ffService = FirefoxDriverService.CreateDefaultService(System.IO.Directory.GetCurrentDirectory());
                    ffService.Port = port;
                    ...
                    _driver = new FirefoxDriver(ffService, FirefoxOptions);
                    break;
            }
        ...
        public void ClickByClass(string classString)
        {
            LogMessage($"Click: {classString}");
            var element = _driver.FindElement(By.ClassName(classString));
            LogMessage($"Click: {classString} - {element}");
            element.Click();
            LogMessage($"Click: {classString} - clicked");
        }
