...
namespace StandingOutStore.Business.Services
{
    public class ClassSessionVideoRoomService : IClassSessionVideoRoomService
    {
        ...
        public async Task<List<DTO.ClassSessionVideoRoomGroup>> Get(Guid classSessionId)
        {
            ...
            List<DTO.ClassSessionVideoRoomGroup> groupData = new List<DTO.ClassSessionVideoRoomGroup>();

            foreach (var room in data.GroupBy(o => o.RoomSid))
            {
                groupData.Add(new DTO.ClassSessionVideoRoomGroup()
                {
                    id = room.Key,
                    Items = Mappings.Mapper.Map<List<Models.ClassSessionVideoRoom>, List<DTO.ClassSessionVideoRoom>>(room.ToList())
                });
            }
